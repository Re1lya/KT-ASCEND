...
run_root=${1:?usage: launch_frozen_p1.sh RUN_ROOT [PORT]}
port=${2:-31000}
...
export SGLANG_KT_NUMERICAL_DUMP_LAYERS=17
...
exec python -m sglang.launch_server \
  ...
  --kt-weight-path /workspace/kt-src/artifacts/round3/deepseek-v2-lite-layer17-f32.gguf \
  --kt-method LLAMAFILE --kt-cpuinfer 16 --kt-threadpool-count 1 --kt-numa-nodes 0 \
  --kt-gpu-experts-ratio 0.9975961538461539 --kt-max-deferred-experts-per-token 0 \
  --kt-expert-placement-strategy frequency \
  --init-expert-location /workspace/kt-src/migration_bridge/round4a/placements/placement_p1.pt \
